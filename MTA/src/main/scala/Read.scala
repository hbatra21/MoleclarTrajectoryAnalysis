...
class Read() extends java.io.Serializable {
  ...
  def read_prm(prm_top_file:String,  total_atoms:Int, total_molecules:Int, crd_file:String, out_dir:String,start_frame: Int, spark: SparkSession) : Dataset[PDB] =
  {
    ...
    var name = sc.parallelize(nmArr,30).zipWithIndex().map{case(a,b) => (b,a)}
    ...
    for(i<- 1 to 9) {
      ...
      name = sc.parallelize(nmArr,30).zipWithIndex().map{case(a,b) => (b,a)}
