...
class Read1 extends java.io.Serializable {
  ...
  def read_pointers(prm_top_file:String,sc: SparkContext):Array[Int]=
  {
    var pointer_flag = 0
    ...
    breakable
    {
      ...
      var atom_flag = 0
      var mol_flag = 0
      val lines = Source.fromFile(prm_top_file).getLines()
      for (line <- lines){
  ...
  def read_prm(prm_top_file:String,  total_atoms:Int, total_molecules:Int, crd_file:String, out_dir:String, sc: SparkContext) : Unit =
  {
    val spark = SparkSession.builder().getOrCreate()
    var start_frame = crd_file.split("/").last.split("_")(1).split('.')(1).toInt + 1
    ...
    val crdRdd = sc.parallelize(crdArr).map { case (x) => x.mkString(",") }.zipWithIndex().map{case(a,b) => (b,a)}
    var temp_name_arr = modifyFrame(name_arr,start_frame)
    var name = sc.parallelize(temp_name_arr).zipWithIndex().map{case(a,b) => (b,a)}
    var temp = name.join(crdRdd).map{case(a,(b,c)) => (b.toString+","+c.toString)}
    //val temp1 = temp.map(row => row(1).toString +","+row(3).toString)
    var tempDs : Dataset[PDB] = temp.map(mapper).toDS()
    ...
    for(i<- 1 to 9) {
      ...
      val crdRdd1 = sc.parallelize(crdArr1).map { case (x) => x.mkString(",") }.zipWithIndex().map{case(a,b) => (b,a)}
      temp_name_arr = modifyFrame(name_arr,start_frame+i)
      name = sc.parallelize(temp_name_arr).zipWithIndex().map{case(a,b) => (b,a)}
      temp = name.join(crdRdd1).map{case(a,(b,c)) => (b.toString+","+c.toString)}
      val temp1 = temp.map(mapper).toDS()
      finalRes = finalRes.union(temp1.filter(temp1("resLabel").=!=("WAT")))//.orderBy($"frameNo",$"index")
    }
  ...
  def readPrmAuto(prm_top_file:String,  total_atoms:Int, total_molecules:Int, crd_file:String, out_dir:String, sc: SparkContext) : Array[Dataset[PDB]] =
  {
    val spark = SparkSession.builder().getOrCreate()
    var start_frame = crd_file.split("/").last.split("_")(1).split('.')(1).toInt + 1
    ...
    val crdArr = Array(X(0), Y(0), Z(0)).transpose
    val crdRdd = sc.parallelize(crdArr,30).map { case (x) => x.mkString(",") }.zipWithIndex().map{case(a,b) => (b,a)}
    var temp_name_arr = modifyFrame(name_arr,start_frame)
    var name = sc.parallelize(temp_name_arr,30).zipWithIndex().map{case(a,b) => (b,a)}
    var temp = name.join(crdRdd).map{case(a,(b,c)) => (b.toString+","+c.toString)}
    //val temp1 = temp.map(row => row(1).toString +","+row(3).toString)
    finalRes(0) = temp.map(mapper).toDS()
    //finalRes.toJavaRDD.coalesce(1).saveAsTextFile("hdfs:///user/ppr.gp2/out/df")

    for(i<- 1 to 9) {
      val crdArr1 = Array(X(i), Y(i), Z(i)).transpose
      val crdRdd1 = sc.parallelize(crdArr1,30).map { case (x) => x.mkString(",") }.zipWithIndex().map{case(a,b) => (b,a)}
      temp_name_arr = modifyFrame(name_arr,start_frame+i)
      name = sc.parallelize(temp_name_arr,30).zipWithIndex().map{case(a,b) => (b,a)}
      temp = name.join(crdRdd1).map{case(a,(b,c)) => (b.toString+","+c.toString)}
      val temp1 = temp.map(mapper).toDS()
    ...
    return finalRes
//    genPdb(1,finalRes, total_atoms, "", spark)
//    return  finalRes
  }
  def genPdb(frameNo:Int, df:Array[PDB],total_atoms: Int, out_dir:String, spark: SparkSession):Unit = {

    var cnt = 0
    var ter_index = 0
    var sc = spark.sparkContext
    //    val pw = new PrintWriter(out_dir+"out" + frameNo + ".pdb")
    ...
    rdd.count()
    var value =  returnFiveDigitNo(frameNo)
    rdd.coalesce(1).saveAsTextFile("hdfs:///user/ppr.gp2/out/pdb_"+value)
    //pw.close()
    //println(out_dir+"out" +frameNo+".pdb created")
  }

  def returnFiveDigitNo(number: Int): String = {
    var fiveDigitNo : String = null
    val length = String.valueOf(number).length
    if (length == 1) fiveDigitNo = "0000" + number.toString
    if (length == 2) fiveDigitNo = "000" + number.toString
    if (length == 3) fiveDigitNo = "00" + number.toString
    if (length == 4) fiveDigitNo = "0" + number.toString
    if (length == 5) fiveDigitNo = number.toString
    fiveDigitNo
  }
