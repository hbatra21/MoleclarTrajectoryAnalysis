...
class Hydrogen_Bond_temp extends Serializable {
...
//    var path = "H_bond_data_file_"+n+".csv"
  ...
  def H_bond(df:Dataset[PDB],Total_Frame:Int, spark:SparkSession) ={
   ...
   // var Total_frames = Total_Frame
    ...
    var df_frame= df.where($"frameNo" === n)
